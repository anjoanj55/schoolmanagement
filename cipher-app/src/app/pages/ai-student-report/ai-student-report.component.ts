...
@Component({
  selector: 'app-ai-student-report',
 standalone: true,
  imports: [
    CommonModule,
    MatTabsModule,
  ...
  templateUrl: './ai-student-report.component.html',
  styleUrl: './ai-student-report.component.css'
})
export class AiStudentReportComponent {
  selectedTabIndex = 0;
  isLoading = false;
  ...
  loadReportByTab(tabIndex: number): void {
    ...
    this.reportName = 'AI Student Academic Report';
  ...
  // const apiUrl = `https://103.199.163.162/ReportsRLSapi/api/Service/LoadPowerBIReportData?` +
    // `reportName=${encodeURIComponent(reportName)}` +
    // `&customerName=${encodeURIComponent(customerName)}` +
    // `&email=${encodeURIComponent(this.reportusername)}` +
    // `&roles=${encodeURIComponent(this.reportrole)}`;
